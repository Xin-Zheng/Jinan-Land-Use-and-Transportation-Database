...
curbparkfield=arcpy.GetParameterAsText(2)
addfieldname=arcpy.GetParameterAsText(3)

arcpy.DeleteField_management(fcpubroad,"curblen")
arcpy.AddField_management(fcpubroad,"curblen","FLOAT")
arcpy.DeleteField_management(fcpubroad,"length")
arcpy.AddField_management(fcpubroad,"length","FLOAT")

fieldexpression="!shape.length!/100*!"+curbparkfield+"!"
arcpy.CalculateField_management (fcpubroad, "curblen" , fieldexpression ,"PYTHON_9.3")
...
spatialjoinsum(fcparcel,fcpubroad,"curblen","sumcurb","CONTAINS")
...
for row in cur: 
    ...
    if newsumlength!=0:
        row.setValue(addfieldname, row.sumcurb/newsumlength)
        cur.updateRow(row)
    else:
        row.setValue(addfieldname, 0)
        cur.updateRow(row)

arcpy.DeleteField_management(fcparcel,["sumcurb","sumlength"])
