...
setbackfield=arcpy.GetParameterAsText(2)
addfieldname=arcpy.GetParameterAsText(3)

arcpy.DeleteField_management(fcpubroad,"calsetback")
arcpy.AddField_management(fcpubroad,"calsetback","FLOAT")
arcpy.DeleteField_management(fcpubroad,"length")
arcpy.AddField_management(fcpubroad,"length","FLOAT")

fieldexpression="!shape.length!*!"+setbackfield+"!"
arcpy.CalculateField_management (fcpubroad, "calsetback" , fieldexpression ,"PYTHON_9.3")
...
spatialjoinsum(fcparcel,fcpubroad,"calsetback","sumsetback","CONTAINS")
...
for row in cur: 
    ...
    if newsumlength!=0:
        row.setValue(addfieldname, row.sumsetback/newsumlength)
        cur.updateRow(row)
    else:
        row.setValue(addfieldname, 0)
        cur.updateRow(row)

arcpy.DeleteField_management(fcparcel,["sumsetback","sumlength"])
