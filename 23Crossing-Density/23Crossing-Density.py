...
crossfield=arcpy.GetParameterAsText(2)
...
spatialjoinsum(fcparcel,fcpubroad,crossfield,"sumcross","CONTAINS")
...
for row in cur: 
    ...
    if newsumlength!=0:
        row.setValue(addfieldname, row.sumcross/newsumlength)
        cur.updateRow(row)
    else:
        row.setValue(addfieldname, 0)
        cur.updateRow(row)

arcpy.DeleteField_management(fcparcel,["sumcross","sumlength"])
