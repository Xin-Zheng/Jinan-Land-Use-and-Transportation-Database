...
highfield=arcpy.GetParameterAsText(2)
widthfield=arcpy.GetParameterAsText(3)
backfield=arcpy.GetParameterAsText(4)
addfieldname=arcpy.GetParameterAsText(5)

arcpy.DeleteField_management(fcpubroad,"hw")
arcpy.AddField_management(fcpubroad,"hw","FLOAT")
...
fieldexpression="!"+highfield+"!/(!"+widthfield+"!+2*!"+backfield+"!)*!length!"
arcpy.CalculateField_management (fcpubroad, "hw" , fieldexpression ,"PYTHON_9.3")
...
spatialjoinsum(fcparcel,fcpubroad,"hw","sumhw","CONTAINS")
...
for row in cur: 
    ...
    if newsumlength!=0:
        row.setValue(addfieldname, row.sumhw/newsumlength)
        cur.updateRow(row)
    else:
        row.setValue(addfieldname, 0)
        cur.updateRow(row)

arcpy.DeleteField_management(fcparcel,["sumhw","sumlength"])
