...
shopfield=arcpy.GetParameterAsText(2)
...
spatialjoinsum(fcparcel,fcpubroad,shopfield,"sumshopnum","CONTAINS")
...
for row in cur: 
    ...
    if newsumlength!=0:
        row.setValue(addfieldname, row.sumshopnum/newsumlength)
        cur.updateRow(row)
    else:
        row.setValue(addfieldname, 0)
        cur.updateRow(row)

arcpy.DeleteField_management(fcparcel,["sumshopnum","sumlength"])
