...
arcpy.DeleteField_management(fcpubroad,"widthlen")
arcpy.AddField_management(fcpubroad,"widthlen","FLOAT")
...
arcpy.CalculateField_management (fcpubroad, "widthlen" , fieldexpression ,"PYTHON_9.3")
...
spatialjoinsum(fcparcel,fcpubroad,"widthlen","sumwidth","CONTAINS")
...
for row in cur: 
    ...
    if newsumlength!=0:
        row.setValue(addfieldname, row.sumwidth/newsumlength)
        cur.updateRow(row)
    else:
        row.setValue(addfieldname, 0)
        cur.updateRow(row)

arcpy.DeleteField_management(fcparcel,["sumwidth","sumlength"])
