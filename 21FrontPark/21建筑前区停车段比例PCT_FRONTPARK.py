...
frontfield=arcpy.GetParameterAsText(2)
addfieldname=arcpy.GetParameterAsText(3)

arcpy.DeleteField_management(fcpubroad,"frontlen")
arcpy.AddField_management(fcpubroad,"frontlen","FLOAT")
arcpy.DeleteField_management(fcpubroad,"length")
arcpy.AddField_management(fcpubroad,"length","FLOAT")

fieldexpression="!shape.length!/100*!"+frontfield+"!"
arcpy.CalculateField_management (fcpubroad, "frontlen" , fieldexpression ,"PYTHON_9.3")
...
spatialjoinsum(fcparcel,fcpubroad,"frontlen","sumfront","CONTAINS")
...
for row in cur: 
    ...
    if newsumlength!=0:
        row.setValue(addfieldname, row.sumfront/newsumlength)
        cur.updateRow(row)
    else:
        row.setValue(addfieldname, 0)
        cur.updateRow(row)

arcpy.DeleteField_management(fcparcel,["sumfront","sumlength"])
