...
illparkfield=arcpy.GetParameterAsText(2)
addfieldname=arcpy.GetParameterAsText(3)

arcpy.DeleteField_management(fcpubroad,"illparklen")
arcpy.AddField_management(fcpubroad,"illparklen","FLOAT")
arcpy.DeleteField_management(fcpubroad,"length")
arcpy.AddField_management(fcpubroad,"length","FLOAT")

fieldexpression="!shape.length!/100*!"+illparkfield+"!"
arcpy.CalculateField_management (fcpubroad, "illparklen" , fieldexpression ,"PYTHON_9.3")
...
spatialjoinsum(fcparcel,fcpubroad,"illparklen","sumill","CONTAINS")
...
for row in cur: 
    ...
    if newsumlength!=0:
        row.setValue(addfieldname, row.sumill/newsumlength)
        cur.updateRow(row)
    else:
        row.setValue(addfieldname, 0)
        cur.updateRow(row)

arcpy.DeleteField_management(fcparcel,["sumill","sumlength"])
