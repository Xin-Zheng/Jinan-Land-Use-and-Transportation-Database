...
fcinroad=arcpy.GetParameterAsText(2)
addfieldname=arcpy.GetParameterAsText(3)

arcpy.AddField_management(fcpubroad,"publength","FLOAT")
arcpy.AddField_management(fcinroad,"inlength","FLOAT")

fieldexpression="!shape.length!"
arcpy.CalculateField_management (fcpubroad, "publength" , fieldexpression ,"PYTHON_9.3")
arcpy.CalculateField_management (fcinroad, "inlength" , fieldexpression ,"PYTHON_9.3")
...
spatialjoinsum(fcparcel,fcpubroad,"publength","sumpub","CONTAINS")
spatialjoinsum(fcparcel,fcinroad,"inlength","sumin","CONTAINS")

arcpy.AddField_management(fcparcel,addfieldname,"FLOAT")

cur = arcpy.UpdateCursor(fcparcel)
for row in cur: 
    sumpubin=row.sumpub+row.sumin
    if sumpubin!=0:
        row.setValue(addfieldname, row.sumpub/(row.sumpub+row.sumin))
        cur.updateRow(row)
    else:
